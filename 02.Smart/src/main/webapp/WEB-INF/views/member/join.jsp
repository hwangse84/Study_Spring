...
<tr>
    ...
    <td><div class="row align-items-center input-check">
          ...
          <div class="col-auto">아이디는 영문 소문자나 숫자 5-10자</div>
          <div class="desc"></div>
       </div>
    </td>
</tr>
   ...
   <td><div class="row">
          <div class="col-auto d-flex file-info align-items-center">
            ...
            <div class="d-flex align-items-center">
               <span class="file-preview"> </span><!-- 미리보기 -->
               <i role="button" class="ms-4 file-delete  d-none  text-danger fs-4 fa-duotone fa-eraser"></i>
            </div>
          </div>
       </div>
    </td>
...
$("#btn-join").click(function(){
	...
	// 중복확인해서 사용가능한 경우만 submit가능!
	var desc=$("[name=user_id]").closest(".input-check").find(".desc").text();
	if(desc.indexOf("사용가능")==-1){//사용가능하지않으면..
		alert("회원가입불가!\n"+desc);
		return;
	}
...
//$(".check-item").keyup(function(){
	...
	if($(this).attr("name")=="user_id" && e.keyCode==13 ){//enter키 코드값이 13이다!
		id_check();
	}else
...
$("#btn-post").click(function(){
 	new daum.Postcode({
 		oncomplete:function(date){
 			...
 			if(data.buildingName !="") adress += "("+data.buildingName+")";
 			//$("[name==address]").eq(0).val(address): 아래처럼 써도됨!
 			$("[name==address]:eq(0)").eq(0).val(address)
 		}
...
$(function(){
	...
	//$("[name=birth]").datepicker("option",{"maxDate",endDay,"showWeek":true}); : 추가속성 지정가능!
})
</script>
