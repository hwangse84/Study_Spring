...
public class CommonUtility {
	private String EMAIL="true2525@naver.com";//관리자의 이메일 주소
	private void connectMailServer(HtmlEmail mail) {
		mail.setDebug(true);
		mail.setCharset("utf-8");
		mail.setHostName("smtp.naver.com");
		mail.setAuthentication("true2525@naver.com","tmddms11!");//관리자의 이메일주소,해당 이메일의 비번
 ...
 //임시 비빌번호를 이메일로 보내기처리
	public boolean sendPassword(MemberVO vo,String pw) {
		...
		connectMailServer(mail);//메인서버에 연결하기( 메일 아이디/비번 입력 후 로그인)
		
		
		try {
			mail.setFrom(EMAIL,"스마트IoT관리자");//메일 보내는 이 정보
 ...
 //웹브라우저 요청의 루트반환
	public String appURL(HttpServletRequest request) {
		...
		url.append(request.getServerName());//http://localhost:,http://127.0.0.01
